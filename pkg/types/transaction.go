...
func NewTransaction(from, to string, amount, fee uint64, symbol, memo string, nonce uint64, chainID uint64) Transaction {
...
func NewContractTransaction(from, to string, fee uint64, symbol string, payload []byte, nonce uint64, chainID uint64) Transaction {
    if envChainID := os.Getenv("BVM_CHAIN_ID"); envChainID != "" {
        if val, err := strconv.ParseUint(envChainID, 10, 64); err == nil {
            chainID = val
        }
...
func NewRegisterTransaction(from, username string, fee uint64, symbol string, nonce uint64, chainID uint64) Transaction {
    if envChainID := os.Getenv("BVM_CHAIN_ID"); envChainID != "" {
        if val, err := strconv.ParseUint(envChainID, 10, 64); err == nil {
            chainID = val
        }
...
func NewRelayerTransaction(mempoolAddr string, actionType string, payload []byte, symbol string, nonce uint64, chainID uint64) Transaction {
    if envChainID := os.Getenv("BVM_CHAIN_ID"); envChainID != "" {
        if val, err := strconv.ParseUint(envChainID, 10, 64); err == nil {
            chainID = val
        }
